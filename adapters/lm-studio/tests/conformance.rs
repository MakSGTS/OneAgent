...
use oneagent_lm_studio::LmStudioProvider;
...
fn provider(base: &str, secret: Option<&str>) -> LmStudioProvider {
...
#[test]
fn public_construction_is_explicit_bounded_local_and_redacted() {
    let sentinel = "synthetic-public-secret-sentinel";
    let accepted = provider("http://127.0.0.1:8080", Some(sentinel));
    assert_eq!(accepted.id().as_str(), "lm-studio");
    let local = LmStudioProvider::new_local(ProviderConfiguration::new(
        ProviderId::new("lm-studio").expect("provider ID must pass"),
        None,
    ))
    .expect("numeric-loopback construction must not perform I/O");
    assert_eq!(local.id().as_str(), "lm-studio");
    ...
    let mismatch = LmStudioProvider::new(
        ProviderConfiguration::new(
    ...
    let invalid_header = LmStudioProvider::new(
        ProviderConfiguration::new(
            ProviderId::new("lm-studio").expect("provider ID must pass"),
            Some(
...
#[tokio::test]
async fn public_native_discovery_and_composed_generation_use_exact_authenticated_wires() {
    let discovery = br#"{
        "models": [
            {"type":"embedding","key":"embed","loaded_instances":[{"id":"ignored"}]},
            {"type":"llm","key":"downloaded","loaded_instances":[{"id":"z"},{"id":"a"}],"future":true},
            {"type":"llm","key":"unloaded","loaded_instances":[]}
        ],
        "future": true
    }"#;
    let completion = r#"{"object":"text_completion","model":"a","choices":[{"text":"ответ","index":0,"finish_reason":"stop"}],"usage":{"ignored":1},"future":true}"#
        .as_bytes();
    ...
    assert_eq!(catalog.provider().as_str(), "lm-studio");
    ...
    let generation = request("lm-studio", "a", "точный вход", "ответ".len());
    let result = provider
        .generate(&generation, context(&policy, &never))
        .await
        .expect("generation must succeed");
    assert_eq!(result.model(), generation.model());
    assert_eq!(result.model().provider().as_str(), "lm-studio");
    ...
    assert!(discovery_request.starts_with("GET /api/v1/models HTTP/1.1\r\n"));
    ...
    assert!(discovery_request.contains("user-agent: oneagent-lm-studio/0.1.0\r\n"));
    ...
    assert!(headers.contains("authorization: Bearer synthetic-public-secret-sentinel"));
    let body: Value =
...
#[tokio::test]
async fn public_empty_catalog_additions_finish_mapping_and_repeated_calls_are_fresh() {
    let responses = vec![
        Some(response("200 OK", br#"{"models":[],"future":true}"#)),
        Some(response(
            "200 OK",
            br#"{"models":[{"type":"llm","key":"parent","loaded_instances":[{"id":"a","future":true}],"future":true}]}"#,
        )),
        Some(response(
            "200 OK",
            br#"{"object":"text_completion","model":"a","choices":[{"text":"x","index":0,"finish_reason":"length"}],"future":true}"#,
        )),
        Some(response(
            "200 OK",
    ...
    let generation = request("lm-studio", "a", "input", 1);
    assert_eq!(
        provider
            .generate(&generation, context(&policy, &never))
            .await
...
#[tokio::test]
async fn public_discovery_rejects_malformed_types_duplicates_counts_and_bounds_atomically() {
    ...
    let cases = vec![
        ...
        (
            br#"{"models":[{"type":"llm","key":"a"}]}"#.to_vec(),
            LlmErrorKind::Protocol,
        ),
        (
            br#"{"models":[{"type":"future","key":"a","loaded_instances":[]}]}"#.to_vec(),
            LlmErrorKind::Protocol,
        ),
        (
            br#"{"models":[{"type":"llm","key":"a","loaded_instances":[{}]}]}"#.to_vec(),
            LlmErrorKind::Protocol,
        ),
        (
            br#"{"models":[{"type":"llm","key":"a","loaded_instances":[{"id":""}]}]}"#.to_vec(),
            LlmErrorKind::InvalidModelCatalog,
        ),
        (
            br#"{"models":[{"type":"llm","key":"a","loaded_instances":[{"id":"same"},{"id":"same"}]}]}"#.to_vec(),
            LlmErrorKind::InvalidModelCatalog,
        ),
        (
            format!(
                r#"{{"models":[{{"type":"llm","key":"parent","loaded_instances":[{over_count}]}}]}}"#
            )
            .into_bytes(),
            LlmErrorKind::InvalidModelCatalog,
        ),
...
#[tokio::test]
async fn public_generation_rejects_fallback_malformed_bounds_status_and_redirect_once() {
    ...
    let cases = vec![
        ...
        (
            response("503 Test", b"provider body"),
            LlmErrorKind::ProviderUnavailable,
        ),
        (
            response("400 Test", b"provider body"),
        ...
        (
            response_without_length("200 OK", &oversized),
            LlmErrorKind::Protocol,
        ),
    ];
    ...
    for (wire, expected) in cases {
        ...
        let adapter = provider(server.base(), None);
        let provider: &dyn LlmProvider = &adapter;
        let request = request("lm-studio", "a", "input", 1);
        let policy = ProviderExecutionPolicy::default();
        let never = NeverCancelled;
        let error = provider
...
async fn public_transport_timeout_cancellation_identity_redaction_and_cleanup_are_terminal() {
    ...
    let llm: &dyn LlmProvider = &adapter;
    let sensitive = request("lm-studio", "a", sentinel, 1);
    let error = llm
...
fn construction_error(result: Result<LmStudioProvider, LlmError>) -> LlmError {
    match result {
