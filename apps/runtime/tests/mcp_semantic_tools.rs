...
use oneagent_runtime::{WorkspaceSnapshotBuilder, graph_semantic_server};
...
fn tool_cases(configuration_id: &str, node_id: &str, diagnostic_configuration: &str) -> Vec<Value> {
    vec![
...
async fn public_graph_tools_are_truthful_policy_gated_and_repeatable() {
    ...
    let diagnostic_configuration = snapshot
        ...
        .as_str()
        .to_owned();
    let server = graph_semantic_server(snapshot).expect("fixed semantic server must build");
    ...
    let cases = tool_cases(&configuration_id, &node_id, &diagnostic_configuration);
...
async fn public_graph_tools_fail_closed_at_argument_and_lookup_boundaries() {
    ...
    for arguments in [
        ...
        json!({"configurationId": "missing", "limit": 1}),
    ] {
        let response = dispatch(
            ...
            &request(
                30,
                "tools/call",
                &json!({"name": "oneagent.validation", "arguments": arguments}),
            ),
        )
        .await;
