...
| 3 | `03-implement-refactoring-plan-domain.md` | Task 2 commit | Immutable typed plan domain and closed validation surface | Non-zero domain identity, ordering, conflict, bound, and error tests | `Implement Sprint 40 refactoring plan domain` |
| 4 | `04-implement-validated-refactoring-planner.md` | Task 3 commit | Deterministic Graph-backed plan evaluation and preview | Non-zero planner, precondition, conflict, completeness, and repetition tests | `Implement Sprint 40 validated refactoring planner` |
| 5 | `05-integrate-workspace-refactoring-plans.md` | Task 4 commit | Immutable Workspace publication composition | Non-zero Workspace snapshot, lifecycle, failure, and source-format tests | `Integrate Sprint 40 Workspace refactoring plans` |
| 6 | `06-integrate-product-refactoring-planning.md` | Task 5 commit | Accepted read-only product projection and process behavior | Non-zero protocol, Tool Policy, MCP, and public-process tests | `Integrate Sprint 40 product refactoring planning` |
| 7 | `07-complete-refactoring-planner-evidence.md` | Task 6 commit | Complete validation and synchronized current-state evidence | Full focused, compatibility, API, dependency, scope, and canonical gates | `Complete Sprint 40 Refactoring Planner evidence` |
| 8 | `08-sprint-40-integration-review.md` | Task 7 commit, no-ff implementation merge into `codex/v0.7`, and review branch | Independent review, reconciliation, state transition, and conditional retirement | Exact immutable range, independent and primary matrices, artifact consistency | `Complete Sprint 40 Refactoring Planner review` |
...
Only Task 8 may conditionally retire this exact immediately preceding suite:
...
- After Task 7, run required validation, merge the sprint branch into
  `codex/v0.7` with `--no-ff`, push the merge, create
  `codex/v0.7-sprint-40-review` from that version head, and dispatch Task 8
  there. After a successful Task 8 commit and push, merge the review branch back
...
Task 8 automatically launches exactly one separately authorized fresh-context
